...
export const Filters: React.FC<Props> = ({
    ...
    return (
        <div className="sticky top-0 z-20">
            <div className="max-w-7xl mx-auto flex flex-col gap-2 px-1">
                <div className="flex flex-wrap gap-2 items-center">
                    {/* Search Bar */}
                    <div className="relative flex-1 w-full md:min-w-[200px] md:max-w-md">
                        ...
                        <input
                            ...
                            className="block w-full pl-9 pr-9 py-2.5 md:py-1.5 bg-dagger-surface border border-dagger-gold/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-dagger-gold text-dagger-light placeholder-dagger-light-dim transition-all shadow-inner text-sm md:text-sm min-h-[44px]"
                            value={search}
                    ...
                    <div className="flex bg-dagger-surface rounded-lg p-1 gap-1 border border-dagger-gold/20">
                        {['1', '2', '3', '4'].map(t => (
                            <button
                                key={t}
                                className={`w-9 h-9 md:w-7 md:h-7 rounded flex items-center justify-center text-sm font-bold transition-all duration-300 font-serif min-h-[44px] md:min-h-0
                                    ${tier === t
                    ...
                    {(role || category || biome || tier || source || search) && (
                        <button
                            ...
                            className="text-xs text-red-400 hover:text-red-300 uppercase tracking-wider font-bold px-3 py-2 md:px-2 md:py-0 min-h-[44px] md:min-h-0"
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`appearance-none text-dagger-light text-sm rounded-lg pl-3 pr-8 py-2.5 md:py-1.5 focus:outline-none focus:border-dagger-gold focus:ring-2 focus:ring-dagger-gold hover:border-dagger-gold/60 transition-colors cursor-pointer min-w-[120px] md:min-w-[120px] text-left flex items-center justify-between min-h-[44px] md:min-h-0 ${
                    isActive 
